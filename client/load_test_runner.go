...
func PerformGetOperation(connection net.Conn, connectionNumber int) {
	connection.Write([]byte("GET count\n"))
	buffer := make([]byte, 1024)
	n, err := connection.Read(buffer)
	if err != nil {
		log.Error().Err(err).Int("connection_number", connectionNumber).Msg("Error reading response")
		return
	}
	...
	if len(responseText) != 0 {
		...
		if err != nil {
			log.Error().Err(err).Int("connection_number", connectionNumber).Msgf("Error converting response to integer: %s", responseText)
			return
		}
	}
...
func PerformSetOperation(connection net.Conn, connectionNumber int) {
	buffer := make([]byte, 1024)
	newCounter := connectionNumber
	connection.Write([]byte("SET count" + fmt.Sprintf(" %d\n", newCounter)))
	_, err := connection.Read(buffer)
	if err != nil {
		log.Error().Err(err).Int("connection_number", connectionNumber).Msgf("Error reading response")
		return
	}
...
const CONCURRENT_CONNECTIONS = 1500
